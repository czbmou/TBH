...
def build_adjacency_hamming(tensor_in):
    ...
    return tf.pow(1 - normalized_dist, 1.383)
...
class TBH(tf.keras.Model):
    def __init__(self, set_name, bbn_dim, cbn_dim, middle_dim=1024, *args, **kwargs):
        ...
        # 判别器的全连接层
        self.fc_1 = tf.keras.layers.Dense(1024, activation='relu') #new layer
        self.fc_2 = tf.keras.layers.Dense(1024, activation='relu') #new layer
    ...
    def call(self, inputs, training=True, mask=None):
        ...
        if training:
            ...
            fc_1_output = self.fc_1(bbn)
            fc_2_output = self.fc_2(bn)
            # 判别器输出
            dis_1 = self.dis_1(fc_1_output)
            dis_2 = self.dis_2(fc_2_output)
            # 解码器输出
            ...
            sample_bn = inputs[2]
            # 判别器对采样数据的输出
            dis_1_sample = self.dis_1(self.fc_1(sample_bbn))
            dis_2_sample = self.dis_2(self.fc_2(sample_bn))
